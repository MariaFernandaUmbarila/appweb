...
public interface EstudianteDetalleRepository extends JpaRepository<EstudianteDetalleModel, Integer> {
    @Query(value = "select en.curso_codigo as codigo, ma.nombre as nombre, sum(en.valor * (en.porcentaje / 100)) as valor from estudiante_detalle en inner join curso cu on(en.curso_codigo = cu.codigo) inner join materia ma on(cu.materia_id = ma.id) where estudiante_id = 1 group by en.curso_codigo, ma.nombre", nativeQuery = true)
